#!/bin/bash
###########################################################################
######## Script for removing entry in FSTAB with LV Name                 ##
...
if [ -s $FILE ];then
        ...
        echo "The $(basename "$FILE") has no values"
        echo ""
...
echo -e "SERVER,LV PRESENT,ENTRY IN FSTAB,STATUS" >> $mailto_file
echo -e "SERVER,LV PRESENT,ENTRY IN FSTAB,STATUS" >> $output_file
...
format_table() {
  ...
  total_width=$((total_width + 3)) # Include leftmost and rightmost border for this script may change for others.
...
delete_entry_fstab() {
    # Backup fstab
    ssh -q -o StrictHostKeyChecking=no -o ConnectTimeout=2 -o BatchMode=yes -p $PORT -l sysadm $server sudo cp -p /etc/fstab /etc/fstab_$(date +%d%m%y%H%M).bkp > /dev/null 2>&1
    # delete the entrieS ENTries from /etc/fstab
    LVLIST=$(ssh -q -o StrictHostKeyChecking=no -o ConnectTimeout=2 -o BatchMode=yes -p $PORT -l sysadm $server sudo grep -i $LVNAME /etc/fstab) > /dev/null 2>&1
        if [ $? = 0 ]; then
        for LVLINE in "$LVLIST";do
                LVNAMES=$(echo "$LVLINE" |awk '{print $1}')
                for LVS in `echo "$LVNAMES"`;do
                LV=$(basename "$LVS")
                echo "$LV" >> $tmpfile
                done
        done

        for ENTRYTOREMOVE in `cat $tmpfile`;do
                ENTRYTOREMOVE=$(echo $ENTRYTOREMOVE | awk -F- '{print $2}')
                ssh -n -tt -q -o StrictHostKeyChecking=no -o ConnectTimeout=2 -o BatchMode=yes -p $PORT -l sysadm $server sudo lvs | grep -i $ENTRYTOREMOVE > /dev/null 2>&1
                if [ $? = 1 ]; then
                        ssh -n -tt -q -o StrictHostKeyChecking=no -o ConnectTimeout=2 -o BatchMode=yes -p $PORT -l sysadm $server \
                        "sudo sed -i '/"$ENTRYTOREMOVE"/d' /etc/fstab"

                        echo -e "$server,NOT Present,Yes,Removed" >> $mailto_file
                        echo -e "$server,NOT Present,Yes,Removed" >> $output_file
                        echo -e "${GREEN} Entry has been removed...${RESET}"
                else
                        echo -e "$server,Present,Yes,No Actions" >> $mailto_file
                        echo -e "$server,Present,Yes,No Actions" >> $output_file
                        echo -e "${BLUE} No Actions needed...${RESET}"
                fi
        done
        else
                echo -e "${RED} No Entry for $LVNAME in FSTAB${RESET}"
                echo -e "$server,NOT Present,NOT Present,No Actions" >> $mailto_file
                echo -e "$server,NOT Present,NOT Present,No Actions" >> $output_file
        fi
}
...
for server in `cat $FILE`;do
        ...
        if [ $? = 0 ] ; then
                PORT=77
                tmpfile=/tmp/$server"_Temp"_$(date +%d%m%y%H%M)
                ...
                rm -rf $tmpfile
                       ...
                       rm -rf $tmpfile
                                ...
                                echo "$server,,,connection KO" >> $output_file
                     fi
        fi
        CNT=`expr $CNT + 1`
done
...
if [[ "${MAILCONF}" =~ ^[Yy] ]];then
        ...
        echo -e "\n\n--------------------------------------------------------------- \n find the attachments; $(if [[ -f "$SCRDIR/INPUTFILES/errorlogs_$FSUF.txt" ]]; then cat $SCRDIR/INPUTFILES/errorlogs_$FSUF.txt; fi)"|mailx -a $mailto_file -r $SENDER -c "msidrm@myorg.com" -s " $0 output file for $DATE" $RECEIVER > /dev/null 2>&1
        echo ""
